...
**Herkomst:** deze werkwijze komt uit [`codebase-standards`](https://github.com/maxonamission/codebase-standards) (v0.4.0; gevendord in `scripts/`, versie-stempel `.codebase-standards-version`, drift bewaakt via `.codebase-standards-manifest.json`). Wijzigingen aan de gedeelde werkwijze lopen via die repo, niet hier.
