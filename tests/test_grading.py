...
class TestBasicMatching:
    ...
    def test_mismatch(self):
        result = grade_answer("est", _make_item("sum"), Taal.LAT)
        assert result.correct is False
        assert result.normalized_answer == "est"
        assert result.normalized_expected == "sum"

    def test_empty_answer_is_never_correct(self):
        assert grade_answer("", _make_item(""), Taal.LAT).correct is False
        assert grade_answer("   ", _make_item("   "), Taal.LAT).correct is False
...
class TestNormalization:
    ...
    def test_case_insensitive_greek(self):
        assert grade_answer("Εἰμί", _make_item("εἰμί"), Taal.GRC).correct is True
...
class TestLatinMacronTolerance:
    ...
    def test_breve_also_stripped(self):
        ...
        assert grade_answer("a", _make_item("ă"), Taal.LAT).correct is True
...
class TestGreekDiacriticsPreserved:
    def test_different_accent_is_incorrect(self):
        ...
        result = grade_answer("εἶμι", _make_item("εἰμί"), Taal.GRC)
        assert result.correct is False
    ...
    def test_nfc_normalization_accepted(self):
        ...
        assert grade_answer(decomposed, _make_item(expected), Taal.GRC).correct is True


class TestListAnswer:
    def test_matches_any_variant(self):
        item = _make_item(["zijn", "bestaan"])
        assert grade_answer("zijn", item, Taal.LAT).correct is True
        assert grade_answer("bestaan", item, Taal.LAT).correct is True
        assert grade_answer("Zijn", item, Taal.LAT).correct is True

    def test_none_matches(self):
        item = _make_item(["zijn", "bestaan"])
        assert grade_answer("zien", item, Taal.LAT).correct is False

    def test_canonical_is_first_variant(self):
        item = _make_item(["zijn", "bestaan"])
        result = grade_answer("bestaan", item, Taal.LAT)
