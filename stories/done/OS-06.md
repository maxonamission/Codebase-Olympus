...
- [x] Break-test: OS-08 van backlog/ naar done/ verplaatst zonder AC af te vinken → script rapporteert 2 fouten (done-AC + status-mismatch), exit 1
...
- [x] Documentatie van story-workflow in CLAUDE.md — **wordt OS-08 (aparte story), hier alleen vermelding van de check-commandos**
...
- Cleanup-commit (`4e07844`):
  ...
  - F1-01..12, B5-01..06, D2-01..07: status `todo`/`backlog` → `done` in EPICS.md
  - F1-12/13/19 + E1-01 als ontbrekende rijen toegevoegd
  - E3-18 umbrella-story: Doel + AC sectie toegevoegd
- Bug-fixes in script tijdens dogfooding:
  - ID-regex `[A-Z]+\d?-\d{1,2}[a-z]?` ondersteunt zowel `A1-01` als `OS-01` als `E3-18a`
...
- Documentatie van de story-workflow zelf hoort thuis in OS-08 (CLAUDE.md-update)
