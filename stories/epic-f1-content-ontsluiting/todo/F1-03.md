...
## Acceptatiecriteria
- [ ] Voor een vocabulaire-knoop (V-type, eerste item) worden de MC-opties uit `stimulus.options` op het scherm getoond en selecteerbaar
- [ ] `stimulus.instruction` wordt als prompt getoond boven de input/opties
- [ ] `stimulus.hint` (bij `luister_productie`) wordt zichtbaar als Nederlandse hint
- [ ] Self-assess fallback blijft werken voor items zonder `options` en zonder `hint`
- [ ] Bestaande tests groen; nieuwe frontend-smoketest voor MC-flow
