# Story F1-04: AudioPlayer component + afspelen van audio_ref in luister-items
...
## Acceptatiecriteria
- [ ] FastAPI serveert `data/audio/` op pad `/audio/{filename}` als static files (read-only)
- [ ] Nieuwe component `frontend/src/components/AudioPlayer.jsx` met play/stop en replay-knop
- [ ] Voor items met `stimulus.audio_ref` wordt de player getoond boven de opties/input
- [ ] Geen autoplay (WCAG/UX); leerling drukt zelf op play
- [ ] Werkt met zowel placeholder-stubs als echte audio (zodra E2 geleverd is)
- [ ] Backend-test: audio-endpoint retourneert 200 + correct content-type; 404 bij onbekend bestand
