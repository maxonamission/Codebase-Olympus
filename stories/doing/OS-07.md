...
- [ ] Eigen agent in `.claude/agents/olympus-reviewer.md` — **expliciet niet in deze story**; volgens story-aanpak eerst ervaring opdoen met standaard-skills, pas een eigen agent schrijven als Olympus-specifieke gaten blijken
- [ ] Voorbeeld-PR handmatig door de workflow heen met `/review` en commentaar vastgelegd — **kan niet vanuit deze sessie**; eerstvolgende PR (deze zelf, na merge) of een daaropvolgende is de natuurlijke test
...
- Eigen agent bewust uitgesteld (zie AC); wachten op eerste live-ervaring met standaard `/review`

## Follow-up
OS-07 blijft in `doing/` tot een echte PR (volgende merge) de review-flow live heeft gedraaid en eventueel commentaar is verwerkt. De eerste natuurlijke test is de merge van de huidige PR.
