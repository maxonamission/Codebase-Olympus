...
### Knopen en data
- [ ] Nieuw graph-bestand `data/graph/lat_integratie_leerjaar1.json` met
...
- [ ] Nieuw graph-bestand `data/graph/grc_integratie_leerjaar1.json` met
      de 3 Griekse equivalenten (`GRC-I-VERT-INTRO`, `-NAAMVAL`, `-POLMO`)
- [ ] `prerequisite`-edges die de integratielaag verankeren:
...
- [ ] Elke `…-I-VERT-NAAMVAL`-knoop heeft ≥3 vertaalitems waar de
      naamval-uitgang het verschil maakt tussen onderwerp en lijdend
      voorwerp (geschikt als latere `diagnose_items` voor M1-02)

### Inhoud en validatie
- [ ] Markdown-content voor elke INTRO-knoop
      (`data/content/LAT-I-VERT-INTRO.md`, `data/content/GRC-I-VERT-INTRO.md`)
      met Begrippen-sectie
- [ ] Volledige graph blijft valide (`validate_graph` groen): geen
      cycles, alle edge-targets resolven, ID's geldig
- [ ] Geen weesknopen: de I-VERT-knopen hangen via prerequisites aan de
      bestaande graph

### Tests
- [ ] Test die bevestigt dat de 6 I-VERT-knopen laden en de graph valide
      blijft (`tests/test_graph_validation.py` of nieuw bestand)
- [ ] Test die bevestigt dat POLMO → I-VERT-POLMO-edge bestaat (de
